...
if st.button("Generate Story & Audio"): with st.spinner("Generating story..."): prompt = f"Write a {length} {genre} roleplay story with characters: {characters}. Split into scenes with dialogue." resp = client.models.generate_content(model=GEMMA_MODEL, contents=[prompt]) story = getattr(resp, 'text', str(resp)) st.session_state["story"] = story
...
st.markdown("---") st.caption("Built with Gemma + Gemini TTS")
