...
if st.button("Generate Story & Comics"): with st.spinner("Writing story..."): prompt = f"Write a {length} {genre} roleplay story with characters: {characters}. Split into scenes with dialogue." resp = client.models.generate_content(model=GEMMA_MODEL, contents=prompt) story = resp.text if hasattr(resp, "text") else str(resp) st.session_state["story"] = story
...
st.markdown("---") st.caption("Built with Gemma + Imagen + Gemini TTS")
