...
with tab2:
    ...
    def stream_mic(ws):
        def callback(indata, frames, time_info, status):
            ...
            try:
                ...
                ws.send(json.dumps({"input_audio_buffer": {"data": audio_b64}}))
            except Exception:
                pass
        ...
        try:
            ...
            st.warning("⚠️ No microphone found.")
            stop_flag.set()
            return

        try:
            with sd.RawInputStream(samplerate=samplerate, blocksize=1024, dtype="int16", channels=1, callback=callback):
                while not stop_flag.is_set():
                    time.sleep(0.05)
        except Exception as e:
            st.warning(f"⚠️ Could not open mic: {e}")
            stop_flag.set()
    ...
    def start_live():
        ...
        setup = {
            "setup": {
                ...
                "generation_config": {
                    ...
                    "speech_config": {
                        "voice_config": {"prebuilt_voice_config": {"voice_name": "Ava"}}
                    },
                },
            }
        }
        ...
        threading.Thread(target=stream_mic, args=(ws,), daemon=True).start()
        threading.Thread(target=receive, args=(ws,), daemon=True).start()

        if st.button("🛑 Stop & Save Session"):
            stop_flag.set()
            ws.close()
            st.success("✅ Session ended. Saving your performance...")
            time.sleep(1)
            if recorded_frames:
                with wave.open("my_live_session.wav", "wb") as wf:
                    wf.setnchannels(1)
                    wf.setsampwidth(2)
                    wf.setframerate(16000)
                    for frame in recorded_frames:
                        wf.writeframes(frame)
                st.audio("my_live_session.wav", format="audio/wav")
                st.success("🎵 Your Live Session is saved as `my_live_session.wav`!")
