...
bill_df = uploaded_df[[bill_col, partyid_col, date_col_master, amount_col_master]].drop_duplicates().reset_index(drop=True) if bill_col and partyid_col and date_col_master and amount_col_master else pd.DataFrame()
...
for table_name, table_df in tables_dict.items():
    ...
    if st.session_state[state_key]:
        ...
        if not table_df.empty:
            ...
            st.download_button(f"⬇️ Download {table_name} (CSV)", data=convert_df_to_csv(table_df), file_name=f"{table_name.lower().replace(' ', '_')}.csv", mime="text/csv")
            st.download_button(f"⬇️ Download {table_name} (Excel)", data=convert_df_to_excel(table_df), file_name=f"{table_name.lower().replace(' ', '_')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        else:
...
if date_col_sel and amount_col_sel:
    try:
        forecast_df = selected_df[[date_col_sel, amount_col_sel]].copy()
        forecast_df[date_col_sel] = pd.to_datetime(forecast_df[date_col_sel], errors="coerce")
    ...
    st.info("ℹ️ Forecasting not available for this table (no Date column).")